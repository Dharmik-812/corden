...
function Thumbnail2D() {
  ...
  useEffect(() => {
    ...
    ctx.strokeStyle = "rgba(71,144,226,0.7)";
    ctx.lineWidth = 1.5;
...
function Thumbnail3D({ seed = 0 }: { seed?: number }) {
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      ctx.strokeStyle = "rgba(71,144,226,0.65)";
      ctx.lineWidth = 1.2;
      edges.forEach(([a, b]) => {
        ctx.beginPath();
...
function ProjectCard({
  ...
  if (view === "list") {
    return (
      ...
      >
        <div
          className={`project-card-new-icon project-card-new-icon--${project.type}`}
        ...
        <div style={{ flex: 1 }}>
          ...
          <p className="project-card-meta">
            <Clock size={10} /> {updatedLabel}
          </p>
        </div>
        <span
          className={`project-card-badge ${badgeClass}`}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {project.starred && (
          <Star size={14} fill="#f59e0b" className="project-card-star" />
        )}
...
function NewProjectCard({ type }: { type: "2d" | "3d" }) {
  return (
    <Link
      href={`/editor/${type}/new`}
      className={`project-card-new project-card-new--${type}`}
    >
      <div className={`project-card-new-icon project-card-new-icon--${type}`}>
        <Plus size={20} />
      </div>
      <div style={{ textAlign: "center" }}>
        <p className="project-card-new-title">
          New {type === "2d" ? "2D Draft" : "3D Model"}
        </p>
    ...
    <div className="page-shell page-shell--dashboard dashboard-hero-bg">
      ...
      <main className="page-content page-content--dashboard">
        ...
        >
          <div className="dashboard-hero-text">
            ...
            <p className="page-desc page-desc--compact">
              {filtered.length} project{filtered.length !== 1 ? "s" : ""} · 2
              presets + saved work
            </p>
          </div>
          <div className="dashboard-stats">
            ...
            >
              <span className="dashboard-stat-value">
                {projects.filter((p) => p.type === "2d").length}
              </span>
            ...
            >
              <span className="dashboard-stat-value">
                {projects.filter((p) => p.type === "3d").length}
              </span>
            ...
            >
              <span className="dashboard-stat-value">
                {projects.filter((p) => p.isPreset).length}
              </span>
        ...
        <motion.div
          ...
          whileHover={{
            boxShadow:
              "0 12px 24px rgba(0,0,0,0.4), inset 0 1px 1px rgba(255,255,255,0.12)",
          }}
        >
          <div
            style={{
          ...
          <div className="dashboard-view-toggle">
            {(
              [
                ["grid", <Grid3X3 key="g" size={14} />],
                ["list", <LayoutList key="l" size={14} />],
              ] as const
            ).map(([v, icon]) => (
              <button
                key={v}
        ...
        {filtered.length === 0 ? (
          ...
          >
            ...
            <p className="project-empty-desc">
              Try a different search or create a new draft.
            </p>
          </motion.div>
        ) : view === "grid" ? (
